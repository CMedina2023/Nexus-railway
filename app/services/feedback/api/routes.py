...
def feedback_validate_project():
    try:
        ...
        valid = FeedbackService(connection).validate_project(project_key)
...
def feedback_submit():
    try:
        ...
        result = FeedbackService(connection).create_feedback_issue(project_key, issue_type, summary, description, user.email)
